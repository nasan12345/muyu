...
function renderPlay(){
  ...
  const b=$('playBody');
  if(s===0){b.innerHTML=`<div style=\"height:100%;display:flex;flex-direction:column\"><img src=\"images/story_lvl${S.l}.png\" style=\"width:100%;height:42%;object-fit:cover;flex-shrink:0\"><div style=\"flex:1;overflow-y:auto;padding:16px 18px\"><div style=\"font-size:11px;color:rgba(255,255,255,.35);letter-spacing:2px;margin-bottom:4px\">${pd().name} · 第${l.id}关</div><div style=\"color:#b8a0c8;font-size:clamp(22px,7vw,30px);font-weight:700;letter-spacing:4px;margin-bottom:4px\">${l.title}</div><div style=\"color:rgba(184,160,200,.4);font-size:clamp(12px,3.5vw,15px);margin-bottom:12px\">${l.subtitle}</div><div style=\"color:rgba(255,255,255,.65);font-size:clamp(13px,3.5vw,16px);line-height:1.8;white-space:pre-line\">${l.story.text}</div></div></div>`}
...
function showResult(){
  ...
  if(score<=7){result='笃行者';accessory='竹简书';desc='一步一印，如竹简上的刻痕。朴拙之中藏大智，一字一句皆是功夫。木鱼说：笃。一下。一下。就够了。';accImg='images/item_bamboo.png';}
  else if(score<=14){result='破局者';accessory='拂尘';desc='拂去迷雾，拨云见日。不惧混沌，敢在乱中找自己的路。木鱼说：扫一扫，就清楚了。';accImg='images/item_whisk.png';}
  else{result='自在者';accessory='葫芦';desc='壶中自有天地。你已不畏外物，不假于形。进退自如，收放随心。木鱼说：乾坤在手，何须多求。';accImg='images/item_gourd.png';}
  var b=$('playBody');
